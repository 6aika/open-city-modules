...
const getEventPlace = (linkedEvent) => {
  if (!linkedEvent.location
    || !linkedEvent.location.name
    || !linkedEvent.location.name.length === 0) {
    return null;
  }
...
const getEventRegion = (linkedEvent, myHelsinkiLocation) => {
  ...
  parsedLocation.latitude = linkedEvent.location &&
                            linkedEvent.location.position &&
                            linkedEvent.location.position.coordinates[1] ||
                            myHelsinkiLocation &&
                            myHelsinkiLocation.lat;
  parsedLocation.longitude = linkedEvent.location &&
                            linkedEvent.location.position &&
                            linkedEvent.location.position.coordinates[0] ||
                            myHelsinkiLocation &&
                            myHelsinkiLocation.lng;
